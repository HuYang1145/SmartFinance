...
public class TransactionChecker {
    ...
    /**
     * 检查给定账户是否存在“异常”交易。
     * 当前定义：异常交易是指类型为 'transfer'（转账）且金额大于 500 的交易。
     *
    ...
    public static boolean hasAbnormalTransactions(AccountModel account) {
        ...
        for (TransactionModel transaction : transactions) {
            ...
            // 6. 应用“异常”判断标准：
            //    - 交易类型是 'transfer' (使用 equalsIgnoreCase 忽略大小写，更健壮)
            //    - 并且 交易金额大于 500
            if ("transfer".equalsIgnoreCase(transaction.getType()) && transaction.getAmount() > 500) {
                // 找到一笔符合条件的异常交易
                System.out.println("TransactionChecker：为账户 " + account.getUsername() + " 找到异常交易 " + transaction.getTransactionId());
