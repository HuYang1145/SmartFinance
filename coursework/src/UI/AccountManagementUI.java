...
public class AccountManagementUI extends JDialog {
    ...
    public AccountManagementUI(JFrame parent, String actionType) {
        ...
        if (actionType.equals("login")) {
            ...
            // 增加账户类型标签的垂直位置，使其与之前的地址字段隔开
            accountTypeLabel.setBounds(labelX, y , width, height);
            panel.add(accountTypeLabel);

            // 增加账户类型下拉框的水平位置，使其与标签有更大的间距
            accountTypeComboBox.setBounds(fieldX , y , width, height);
            panel.add(accountTypeComboBox);

            // 增加账户类型选择后与注册按钮之间的垂直间距
            y += 60; // 原来的 gap 是 50，这里增加到 60
    ...
    private void loginAccount(String username, String password) {
        ...
        for (AccountModel account : accounts) {
            if (account.getUsername().equals(username) && account.getPassword().equals(password)) {
                if (account.getAccountStatus().equals("FROZEN")) {
                    JOptionPane.showMessageDialog(this, "This account has been frozen due to suspicious activity. Please contact the administrator to resolve this.", "Account Frozen", JOptionPane.ERROR_MESSAGE);
                    return; // Prevent login if account is frozen
                } else {
                    loginSuccess = true;
        ...
        } else if (!loginSuccess) { // Only show this message if not already shown the frozen message
