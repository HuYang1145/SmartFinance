...
import Person.ViewPersonalInfo; // 确保这个类名是正确的
import Person.WithdrawalDialog;
import Person.transferAccounts;
import java.awt.*; // 确保 UserSession 类及其方法可用
...
public class PersonalUI extends JDialog {
    ...
    public PersonalUI() {
        ...
        JButton monthlyIncomeExpenseButton = createFunctionButton("月度收入支出分类", "/UI/icons/chart.png", btnSize);
        ...
        sidebarPanel.add(Box.createRigidArea(new Dimension(0, 15)));
        sidebarPanel.add(monthlyIncomeExpenseButton); //
        ...
        //分类按钮
        monthlyIncomeExpenseButton.addActionListener(e -> {
            // 假设 CSV 文件路径为 "tractions.csv"
