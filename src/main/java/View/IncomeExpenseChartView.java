...
import javax.swing.table.DefaultTableCellRenderer; // 导入 TransactionController
import javax.swing.table.JTableHeader; // 导入 TransactionModel

import AccountModel.BudgetServiceModel; // 导入 BudgetServiceModel 以使用 DATE_FORMATTER
    ...
    private static JPanel createDonutChartPanel(String username, boolean isExpense, String yearMonth) {
        return new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                List<TransactionModel> transactions = getFilteredTransactions(username, yearMonth);
 ...
 */
private static Map<String, Double> calculateExpenseCategoryTotals(List<TransactionModel> transactions) {
 ...
 */
private static Map<String, Double> calculateIncomeCategoryTotals(List<TransactionModel> transactions) {
    Map<String, Double> categoryTotals = new HashMap<>();
    for (TransactionModel tx : transactions) {
...
public static JPanel getTransactionTablePanel(String username, String yearMonth) {
    ...
    for (int i = 0; i < transactions.size(); i++) {
        ...
        data[i] = new Object[] {
                t.getAccountUsername(), t.getOperation(), String.format("%.2f", t.getAmount()), t.getTimestamp(),
...
public static JPanel getDailyLineChartPanel(String username, String yearMonth) {
    return new JPanel(new BorderLayout(10, 10)) {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (TransactionModel t : transactions) {
                try {
                    LocalDate date = LocalDate.parse(t.getTimestamp(), BudgetServiceModel.DATE_FORMATTER);
