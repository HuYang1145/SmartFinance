...
public class PersonalMainPlane extends JFrame {
    ...
    private JPanel createBillStatisticsPanel() {
        ...
        JPanel panel = new GradientPanel(); // Assumes GradientPanel is defined elsewhere
        ...
        // Initialize chart with current year/month
        java.util.Calendar cal = java.util.Calendar.getInstance();
        String currentYearMonth = String.format("%d/%02d", cal.get(java.util.Calendar.YEAR), cal.get(java.util.Calendar.MONTH) + 1);

        try {
            chartPanel = ChartController.getIncomeExpensePlane(username, currentYearMonth);
            panel.add(chartPanel, BorderLayout.CENTER);
        } catch (Exception e) {
            e.printStackTrace();
            JLabel errorLabel = new JLabel("Failed to load chart: " + e.getMessage(), SwingConstants.CENTER);
            errorLabel.setForeground(Color.RED);
            panel.add(errorLabel, BorderLayout.CENTER);
            chartPanel = errorLabel;
        }

        // Selector panel for year/month
        ...
        ActionListener updateListener = e -> {
            try {
                ...
                JPanel newChartPanel = ChartController.getIncomeExpensePlane(username, selectedYearMonth);
    ...
    public static void main(String[] args) {
        String userId = UserSessionModel.getCurrentUsername(); // Code 1 logic
        if (userId == null) {
