...
public class TransactionService {
    ...
    /**
     * Appends a new transaction record to the transactions.csv file with minimal required fields.
     *
     * @param username  The username associated with the transaction.
     * @param operation The operation type ("Income" or "Expense").
     * @param amount    The transaction amount.
     * @param time      The transaction time (format: yyyy/MM/dd HH:mm).
    ...
    /**
     * Appends a new transaction record to the transactions.csv file with all fields.
     */
    public static boolean addTransaction(String username, String operation, double amount, String time, String merchant, String type,
        ...
        // Validate date/time format as yyyy/MM/dd HH:mm
    ...
    /**
     * Removes a transaction record from transactions.csv based on username and time.
     *
     * @param username The username associated with the transaction.
     * @param time     The transaction time (format: yyyy/MM/dd HH:mm).
    ...
    public static boolean removeTransaction(String username, String time) {
        ...
        if (!file.exists() || username == null || username.trim().isEmpty() || time == null || time.trim().isEmpty()) {
            System.err.println("Invalid parameters or file does not exist for removeTransaction.");
            return false;
        }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file, StandardCharsets.UTF_8));
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (data.length >= 4 && username.equals(unescapeCsvField(data[0])) && time.equals(unescapeCsvField(data[3]))) {
                    removed = true;
                    continue; // Skip the matching transaction
                }
                bw.write(line);
                bw.newLine();
            }
        } catch (IOException e) {
            System.err.println("Error removing transaction from CSV: " + e.getMessage());
            JOptionPane.showMessageDialog(null,
                    "Failed to remove transaction: " + e.getMessage(),
                    "Transaction Error", JOptionPane.ERROR_MESSAGE);
            tempFile.delete();
            return false;
        }
